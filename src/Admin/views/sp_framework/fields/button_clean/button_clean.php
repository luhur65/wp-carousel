...
if ( ! class_exists( 'SP_WPCP_Framework_Field_button_clean' ) ) {
	 ...
	 * @version 1.0.0
	 */
	class SP_WPCP_Framework_Field_button_clean extends SP_WPCP_Framework_Fields {
		/**
			...
			if ( isset( $this->field['options'] ) ) {
				...
				if ( is_array( $options ) && ! empty( $options ) ) {
					...
					foreach ( $options as $key => $option ) {
						...
						echo '<button class="sp_wpcp--sibling sp_wpcp-confirm button-primary sp_wpcp--button' . esc_attr( $active ) . '" data-confirm="' . esc_html__( 'Are you sure to clean cache?', 'wp-carousel-pro' ) . '">';
